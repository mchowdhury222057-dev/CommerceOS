...
function LogoMark({ storeName, logoUrl }: { storeName: string; logoUrl: string | null }) {
  if (logoUrl) {
    return <img src={logoUrl} alt={storeName} className="h-9 w-9 rounded-md object-cover" />;
  }
  return (
...
export function Nav({ storeName, logoUrl }: { storeName: string; logoUrl: string | null }) {
    ...
    <header className="sticky top-0 z-20 border-b border-border-default bg-surface-card">
      <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-3">
        ...
        {/* Visual-only search - not functionally wired this round (out of
            ...
            real storefront, not a placeholder. */}
        <div className="hidden flex-1 md:block">
          <label className="relative block">
            ...
            <input
              ...
              className="w-full max-w-md rounded-full border border-border-default bg-surface-page py-2 pl-9 pr-4 text-sm text-text-primary placeholder:text-text-disabled focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
            />
          </label>
        </div>
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          <Link
            ...
            className="hidden rounded-md px-3 py-2 text-sm font-medium text-text-secondary hover:bg-surface-sunken hover:text-text-primary sm:block"
          >
            Track Order
          </Link>
          <Link
            ...
            className="relative flex h-10 w-10 items-center justify-center rounded-md text-text-primary hover:bg-surface-sunken"
          >
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-md text-text-primary hover:bg-surface-sunken md:hidden"
          >
        ...
        <div className="border-t border-border-default bg-surface-card px-4 py-3 md:hidden">
          <label className="relative mb-2 block">
            ...
            <input
              type="search"
              placeholder={`Search ${storeName}…`}
              className="w-full rounded-full border border-border-default bg-surface-page py-2 pl-9 pr-4 text-sm text-text-primary placeholder:text-text-disabled focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
            />
          </label>
          <Link
            to={`/${storeSlug}/track-order`}
            onClick={() => setMenuOpen(false)}
            className="block rounded-md px-3 py-2 text-sm font-medium text-text-secondary hover:bg-surface-sunken hover:text-text-primary"
          >
            Track Order
          </Link>
        </div>
