...
export interface TableProps {
  children: ReactNode;
  /** Renders inside the same bordered/rounded container, below the table
   * (e.g. Pagination) - so it visually attaches as one card instead of a
   * separate borderless strip underneath. */
  footer?: ReactNode;
}

export function Table({ children, footer }: TableProps) {
  return (
...
export function TR({ className, clickable = false, ...rest }: TRProps) {
  return <tr className={cn("transition-colors hover:bg-surface-sunken/60", clickable && "cursor-pointer", className)} {...rest} />;
}
